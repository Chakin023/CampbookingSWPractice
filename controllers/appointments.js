...
//@route POST /api/v1/campbookings/:campbookingId/appointment
//@access Private
exports.addAppointment = async (req, res, next) => {
    try {
        req.body.campbooking = req.params.campbookingId;
        const campbooking = await Campbooking.findById(req.params.campbookingId);
    if (!campbooking) {
...
exports.updateAppointment = async (req, res, next) => {
    ...
    try {
        ...
        if(appointment.user.toString()!== req.user.id && req.user.role !== 'admin'){
            return res.status(401).json({success:false,message:`User ${req.user.id} is not autherized to update this appointment`});
        }
